...
func main() {
	...
	str := "https://api.powerbi.com/v1.0/myorg/groups?workspaceV2=%s"
	fmt.Println(fmt.Sprintf(str, "true"))

	client := &http.Client{}
	req, err := http.NewRequest("POST", common.POWERBI_REST_POST_GROUPS2, nil)
	if err != nil {

	}
	query := req.URL.Query()
	query.Add("workspaceV2", "true")
